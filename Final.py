...
PLOTS_DIR = "optimization_plots_Final"
...
def generate_diagnostic_subplots(estimator_class, base_params,
    ...
    fig, axs = plt.subplots(2, 3, figsize=(24, 14),dpi=300)
...
def run_combined_workflow():
            ...
            if num_obj_plot < 2:
                ...
                for i, name in enumerate(active_objective_names_for_pymoo):
                    ...
                    sc = ax.scatter(XY_radviz[:, 0], XY_radviz[:, 1], c=pareto_F[:, i], cmap="viridis", s=60, ec="k",
                                    alpha=0.8, label="Pareto Designs")
                    poly_x = np.append(anchors[:, 0], anchors[0, 0]);
                    poly_y = np.append(anchors[:, 1], anchors[0, 1])
                    ax.plot(poly_x, poly_y, "--", c="r", lw=2.0, label="Objective Anchors")
                    ax.scatter(anchors[:, 0], anchors[:, 1], marker="^", c="r", s=120)
                    label_offset_scale = 0.05  # Adjust to bring labels closer/farther

                    # Optional manual tweaks for overlapping labels
                    nudges = {
                        "Corner BGA DeltaW": (-0.09, -0.07),
                        "Corner Bump DeltaW": (-0.14, 0.01),
                        "Warpage at BGA max reflow": (0.02, -0.03),
                        "Theta JA": (0.06, -0.04), "ELK stress": (0.06, 0.04),
                        # Add other labels as needed
                    }

                    for il, lab in enumerate(active_objective_names_for_pymoo):
                        x, y = anchors[il, 0], anchors[il, 1]

                        # Unit direction vector to keep spacing consistent
                        norm = np.sqrt(x ** 2 + y ** 2)
                        dx = (x / norm) * label_offset_scale
                        dy = (y / norm) * label_offset_scale

                        dx_nudge, dy_nudge = nudges.get(lab, (0.0, 0.0))
                    ...
                    ax.set_title(f"RadViz (Color by {name})", fontsize=13, fontweight='bold')
                    ax.set_xlabel("RadViz X", fontsize=11)
                    ax.set_ylabel("RadViz Y", fontsize=11)
                    ...
                    plt.savefig(os.path.join(PLOTS_DIR, f"radviz_opt_{sfname}.png"), dpi=300)  # High-resolution output
                    plt.close(fig)
    ...
    if pareto_F is not None and len(pareto_F) > 0 and pareto_F.shape[1] >= 2:
        for i1, i2 in itertools.combinations(range(pareto_F.shape[1]), 2):
            ...
            fig_scatter_s, ax_scatter_s = plt.subplots(figsize=(9, 8),dpi=300)
            ...
            plt.savefig(os.path.join(PLOTS_DIR, f"scatter_opt_{stitle}.png"),dpi=300);
    ...
    if num_all_targets > 0:
        fig_all_dist, axs_all_dist = plt.subplots(num_all_targets, 2, figsize=(14, 5 * num_all_targets), squeeze=False,dpi=300)
    ...
    if num_fi_plots > 0:
        fig_fi, axs_fi = plt.subplots(num_fi_plots, 1, figsize=(12, 7 * num_fi_plots), squeeze=False,dpi=300)
